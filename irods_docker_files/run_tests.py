...
def run_tests(image_name, irods_repo, irods_commitish, build_dir, output_directory, database_type, test_name_prefix):
    run_tests_cmd = ['python run_tests_in_parallel.py --image_name {0} --jenkins_output {1} --test_name_prefix {2} -b {3} --database_type {4} --irods_repo {5} --irods_commitish {6}'.format(image_name, output_directory, test_name_prefix, build_dir, database_type, irods_repo, irods_commitish)]
...
def main():
    ...
    if not args.test_plugin:
        run_tests(build_tag, args.irods_repo, args.irods_commitish, args.irods_build_dir, args.output_directory, args.database_type, test_name_prefix)
    else:
